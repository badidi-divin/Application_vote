...
if(isset($_GET['op']) and $_GET['op']=="add"){
	...
	$bd_unite_stock = htmlspecialchars(trim($_GET['unite_stock']));
	...
	if($count=$req1->rowcount()<=0){
		$date_ajout=date('d-m-Y H:i:s');
		$req=$bdd->prepare("INSERT INTO t_produits SET ref_prod=?, designation=?,Descript=?,prix_gros=?,prix_unit=?,stock=?,unite=?,date_prod=?,date_creat=?,Lib_cat=?,etat=?");
		$req->execute(array($bd_ref_prod,$bd_desi,$bd_descrip,$bd_prix_achat,$bd_prix_unit,$bd_qte_stock,$bd_unite_stock,$bd_date_prod,$date_ajout,$bd_categ,"1"));

		$json['message']="OK";	
		$json['prod_info']="Référence : ".$bd_ref_prod."| Article : ".strtoupper($bd_desi); 
...
if(isset($_GET['ref_prod2'])){
	
	$bd_ref_prod= htmlspecialchars(trim($_GET['ref_prod2']));
    $bd_desi= htmlspecialchars(trim($_GET['desi']));
	$bd_descrip= htmlspecialchars(trim($_GET['descrip']));
	//$bd_dci= htmlspecialchars(trim($_GET['dci']));
	$bd_prix_achat= htmlspecialchars(trim($_GET['prix_achat']));
	...
	$bd_categ= htmlspecialchars(trim($_GET['categ']));
		...
		$date_ajout=date('d-m-Y H:i:s');
		$req=$bdd->prepare("UPDATE t_produits SET ref_prod=?, designation=?,Descript=?,prix_gros=?,prix_unit=?,stock=?, unite=?,date_prod=?,Lib_cat=?, date_creat=? where ref_prod=?");
		$req->execute(array($bd_ref_prod,$bd_desi,$bd_descrip,$bd_prix_achat,$bd_prix_unit,$bd_qte_stock,$bd_unite_stock,$bd_date_prod,$bd_categ,$date_ajout,$bd_ref_prod));
         
        header('location:../medicaments.php?msg=2');

		$json['message']="OK";	
		$json['prod_info']="Réf prod : ".$bd_ref_prod." | Designation : ".strtoupper($bd_desi)." | Stock : "
		.$bd_qte_stock." ".$bd_unite_stock." | Prix gros : ".$bd_prix_achat." $ | Prix unitaire : ".$bd_prix_unit." Fc | Categorie : ".$bd_categ; 
}
...
if(isset($_GET['spr_prd'])){
	 ...
	 $idpn=$_GET['spr_prd'];
	 $query3=$bdd->prepare("UPDATE t_produits SET etat=0 where id=?");
